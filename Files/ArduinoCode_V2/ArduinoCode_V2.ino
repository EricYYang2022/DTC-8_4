...
static int cal_val_0 = 281;
static int cal_val_1 = 347;
static int cal_val_2 = 622;
static int cal_val_3 = 470;
...
void loop() {
  ...
  if (summ > 40) {
    ...
    isOnBoard = 0;
    Serial.println("You are not on the board!");
  }
  ...
  if (isOnBoard == 1) {
    ...
    if (abs(diff0 + diff1 + diff2 + diff3) <= equilibriumVal) {
      Serial.print("I pressed n, btw");
      Keyboard.press('n');
      delay(50);
      Keyboard.release('n');
    }
    else {
      Serial.print("I pressed m, btw");
      Keyboard.press('m');
      delay(50);
      Keyboard.release('m');
    }
